...
(function (Endabgabe) {
    class Ball {
        ...
        setnewPosition(_newPosition) {
            ...
            let chosenPlayer = Endabgabe.humans[this.playerIndex];
            ...
            if (random >= 0.5) {
                ...
                newX = _newPosition.x - ((distanceBall / this.precisionChecker) * chosenPlayer.playerPrecision); //genauer Treffer 
            }
            random = Math.random();
            if (random >= 0.5) {
        ...
        update() {
            if (Endabgabe.key == true) {
                // Hier kein neues "new" notwendig, da Vector.getDifference einen Returnwert hat
                // Die Klassen sind alles Blaupausen, wo ein gewisser Wert abgerufen wird. Die Variablen hier zeigen nur darauf.
