    ...
    function createPlayer(): void {
        for (let indexA: number = 0; indexA < 2; indexA++) {
            for (let indexB: number = 0; indexB < 11; indexB++) {
                ...
                if (indexA == 0) {
                    humans.push(new Player(new Vector(positionsTeam1[indexB].x, positionsTeam1[indexB].y), "red", true, randomNumber, "A"));
                }
                else {
                    humans.push(new Player(new Vector(positionsTeam2[indexB].x, positionsTeam2[indexB].y), "blue", true, randomNumber, "B"));
                }
            }
        }
        ...
        for (let indexA: number = 0; indexA < 2; indexA++) {
            for (let indexB: number = 11; indexB < 14; indexB++) {
                ...
                let randomNumber: number = Math.floor(1 + Math.random() * (50 - 1));
                if (indexA == 0) {
                    humans.push(new Player(new Vector(positionsTeam1[indexB].x, positionsTeam1[indexB].y), "red", false, randomNumber, "A"));
                }
                else {
                    humans.push(new Player(new Vector(positionsTeam2[indexB].x, positionsTeam2[indexB].y), "blue", false, randomNumber, "B"));
                }
            }
        }
    }
